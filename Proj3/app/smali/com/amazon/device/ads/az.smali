.class Lcom/amazon/device/ads/az;
.super Ljava/lang/Object;


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/amazon/device/ads/az$a;
    }
.end annotation


# static fields
.field private static final a:Ljava/lang/String;

.field private static b:Lcom/amazon/device/ads/az;


# instance fields
.field private c:Ljava/lang/String;

.field private final d:Lcom/amazon/device/ads/ct;

.field private final e:Lcom/amazon/device/ads/bw;

.field private final f:Lcom/amazon/device/ads/ea;

.field private final g:Lcom/amazon/device/ads/cr;

.field private final h:Lcom/amazon/device/ads/cu;


# direct methods
.method static constructor <clinit>()V
    .locals 5

    const-class v0, Lcom/amazon/device/ads/az;

    invoke-virtual {v0}, Ljava/lang/Class;->getSimpleName()Ljava/lang/String;

    move-result-object v0

    sput-object v0, Lcom/amazon/device/ads/az;->a:Ljava/lang/String;

    new-instance v0, Lcom/amazon/device/ads/az;

    invoke-static {}, Lcom/amazon/device/ads/ct;->a()Lcom/amazon/device/ads/ct;

    move-result-object v1

    new-instance v2, Lcom/amazon/device/ads/bm;

    invoke-direct {v2}, Lcom/amazon/device/ads/bm;-><init>()V

    invoke-static {}, Lcom/amazon/device/ads/ea;->a()Lcom/amazon/device/ads/ea;

    move-result-object v3

    invoke-static {}, Lcom/amazon/device/ads/cr;->a()Lcom/amazon/device/ads/cr;

    move-result-object v4

    invoke-direct {v0, v1, v2, v3, v4}, Lcom/amazon/device/ads/az;-><init>(Lcom/amazon/device/ads/ct;Lcom/amazon/device/ads/bw;Lcom/amazon/device/ads/ea;Lcom/amazon/device/ads/cr;)V

    sput-object v0, Lcom/amazon/device/ads/az;->b:Lcom/amazon/device/ads/az;

    return-void
.end method

.method protected constructor <init>(Lcom/amazon/device/ads/ct;Lcom/amazon/device/ads/bw;Lcom/amazon/device/ads/ea;Lcom/amazon/device/ads/cr;)V
    .locals 2

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Lcom/amazon/device/ads/cv;

    invoke-direct {v0}, Lcom/amazon/device/ads/cv;-><init>()V

    sget-object v1, Lcom/amazon/device/ads/az;->a:Ljava/lang/String;

    invoke-virtual {v0, v1}, Lcom/amazon/device/ads/cv;->a(Ljava/lang/String;)Lcom/amazon/device/ads/cu;

    move-result-object v0

    iput-object v0, p0, Lcom/amazon/device/ads/az;->h:Lcom/amazon/device/ads/cu;

    iput-object p1, p0, Lcom/amazon/device/ads/az;->d:Lcom/amazon/device/ads/ct;

    iput-object p2, p0, Lcom/amazon/device/ads/az;->e:Lcom/amazon/device/ads/bw;

    iput-object p3, p0, Lcom/amazon/device/ads/az;->f:Lcom/amazon/device/ads/ea;

    iput-object p4, p0, Lcom/amazon/device/ads/az;->g:Lcom/amazon/device/ads/cr;

    invoke-virtual {p0}, Lcom/amazon/device/ads/az;->b()Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, Lcom/amazon/device/ads/az;->c:Ljava/lang/String;

    return-void
.end method

.method public static final a()Lcom/amazon/device/ads/az;
    .locals 1

    sget-object v0, Lcom/amazon/device/ads/az;->b:Lcom/amazon/device/ads/az;

    return-object v0
.end method

.method private a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V
    .locals 1

    invoke-static {p2}, Lcom/amazon/device/ads/ba;->a(Ljava/lang/String;)[B

    move-result-object v0

    invoke-direct {p0, p1, v0, p3}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;[BLjava/lang/String;)V

    return-void
.end method

.method private a(Lcom/amazon/device/ads/az$a;[BLjava/lang/String;)V
    .locals 5

    invoke-static {p1}, Lcom/amazon/device/ads/az$a;->a(Lcom/amazon/device/ads/az$a;)Z

    move-result v0

    if-eqz v0, :cond_0

    :goto_0
    return-void

    :cond_0
    iget-object v0, p0, Lcom/amazon/device/ads/az;->e:Lcom/amazon/device/ads/bw;

    invoke-interface {v0, p3}, Lcom/amazon/device/ads/bw;->a(Ljava/lang/String;)Lcom/amazon/device/ads/by;

    move-result-object v0

    if-nez v0, :cond_1

    iget-object v0, p0, Lcom/amazon/device/ads/az;->h:Lcom/amazon/device/ads/cu;

    const-string v1, "Could not create output handler for asset file."

    invoke-virtual {v0, v1}, Lcom/amazon/device/ads/cu;->f(Ljava/lang/String;)V

    goto :goto_0

    :cond_1
    sget-object v1, Lcom/amazon/device/ads/by$a;->b:Lcom/amazon/device/ads/by$a;

    invoke-virtual {v0, v1}, Lcom/amazon/device/ads/by;->a(Lcom/amazon/device/ads/by$a;)Z

    move-result v1

    if-eqz v1, :cond_2

    :try_start_0
    invoke-virtual {v0, p2}, Lcom/amazon/device/ads/by;->a([B)V
    :try_end_0
    .catch Ljava/io/IOException; {:try_start_0 .. :try_end_0} :catch_0

    :goto_1
    invoke-virtual {v0}, Lcom/amazon/device/ads/by;->close()V

    goto :goto_0

    :catch_0
    move-exception v1

    iget-object v1, p0, Lcom/amazon/device/ads/az;->h:Lcom/amazon/device/ads/cu;

    const-string v2, "Caught IOException while writing asset to disk: %s"

    const/4 v3, 0x1

    new-array v3, v3, [Ljava/lang/Object;

    const/4 v4, 0x0

    aput-object p3, v3, v4

    invoke-virtual {v1, v2, v3}, Lcom/amazon/device/ads/cu;->d(Ljava/lang/String;[Ljava/lang/Object;)V

    invoke-static {p1}, Lcom/amazon/device/ads/az$a;->b(Lcom/amazon/device/ads/az$a;)V

    goto :goto_1

    :cond_2
    invoke-static {p1}, Lcom/amazon/device/ads/az$a;->b(Lcom/amazon/device/ads/az$a;)V

    goto :goto_1
.end method

.method private d()Z
    .locals 3

    new-instance v0, Lcom/amazon/device/ads/az$a;

    invoke-direct {v0, p0}, Lcom/amazon/device/ads/az$a;-><init>(Lcom/amazon/device/ads/az;)V

    const-string v1, ""

    const-string v2, "amazon_ads_close_normal.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_close_pressed.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_close.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_leftarrow.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_refresh.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_rightarrow.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_close_normal.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_close_pressed.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    const-string v1, ""

    const-string v2, "amazon_ads_open_external_browser.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/az;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/az;->a(Lcom/amazon/device/ads/az$a;Ljava/lang/String;Ljava/lang/String;)V

    invoke-static {v0}, Lcom/amazon/device/ads/az$a;->a(Lcom/amazon/device/ads/az$a;)Z

    move-result v0

    if-nez v0, :cond_0

    const/4 v0, 0x1

    :goto_0
    return v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method


# virtual methods
.method public a(Ljava/lang/String;)Ljava/lang/String;
    .locals 2

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    iget-object v1, p0, Lcom/amazon/device/ads/az;->c:Ljava/lang/String;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method protected b()Ljava/lang/String;
    .locals 2

    iget-object v0, p0, Lcom/amazon/device/ads/az;->d:Lcom/amazon/device/ads/ct;

    if-eqz v0, :cond_0

    iget-object v0, p0, Lcom/amazon/device/ads/az;->d:Lcom/amazon/device/ads/ct;

    invoke-virtual {v0}, Lcom/amazon/device/ads/ct;->h()Ljava/io/File;

    move-result-object v0

    if-eqz v0, :cond_0

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    iget-object v1, p0, Lcom/amazon/device/ads/az;->d:Lcom/amazon/device/ads/ct;

    invoke-virtual {v1}, Lcom/amazon/device/ads/ct;->h()Ljava/io/File;

    move-result-object v1

    invoke-virtual {v1}, Ljava/io/File;->getAbsolutePath()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    sget-object v1, Ljava/io/File;->separator:Ljava/lang/String;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    :goto_0
    return-object v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public declared-synchronized c()Z
    .locals 9

    const/4 v7, 0x5

    const/4 v1, 0x1

    const/4 v0, 0x0

    monitor-enter p0

    :try_start_0
    iget-object v2, p0, Lcom/amazon/device/ads/az;->c:Ljava/lang/String;

    if-nez v2, :cond_0

    invoke-virtual {p0}, Lcom/amazon/device/ads/az;->b()Ljava/lang/String;

    move-result-object v2

    iput-object v2, p0, Lcom/amazon/device/ads/az;->c:Ljava/lang/String;

    iget-object v2, p0, Lcom/amazon/device/ads/az;->c:Ljava/lang/String;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    if-nez v2, :cond_0

    :goto_0
    monitor-exit p0

    return v0

    :cond_0
    :try_start_1
    invoke-static {}, Ljava/lang/System;->nanoTime()J

    move-result-wide v2

    iget-object v4, p0, Lcom/amazon/device/ads/az;->f:Lcom/amazon/device/ads/ea;

    const-string v5, "mraid-assets-version"

    const/4 v6, 0x0

    invoke-virtual {v4, v5, v6}, Lcom/amazon/device/ads/ea;->a(Ljava/lang/String;I)I

    move-result v4

    if-ge v4, v7, :cond_1

    iget-object v4, p0, Lcom/amazon/device/ads/az;->h:Lcom/amazon/device/ads/cu;

    const-string v5, "Writing assets to disk: version %d"

    const/4 v6, 0x1

    new-array v6, v6, [Ljava/lang/Object;

    const/4 v7, 0x0

    const/4 v8, 0x5

    invoke-static {v8}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    aput-object v8, v6, v7

    invoke-virtual {v4, v5, v6}, Lcom/amazon/device/ads/cu;->c(Ljava/lang/String;[Ljava/lang/Object;)V

    invoke-direct {p0}, Lcom/amazon/device/ads/az;->d()Z

    move-result v4

    if-eqz v4, :cond_2

    iget-object v0, p0, Lcom/amazon/device/ads/az;->f:Lcom/amazon/device/ads/ea;

    const-string v4, "mraid-assets-version"

    const/4 v5, 0x5

    invoke-virtual {v0, v4, v5}, Lcom/amazon/device/ads/ea;->b(Ljava/lang/String;I)V

    move v0, v1

    :cond_1
    invoke-static {}, Ljava/lang/System;->nanoTime()J

    move-result-wide v4

    sub-long v2, v4, v2

    invoke-static {v2, v3}, Lcom/amazon/device/ads/da;->a(J)J

    move-result-wide v2

    iget-object v4, p0, Lcom/amazon/device/ads/az;->g:Lcom/amazon/device/ads/cr;

    invoke-virtual {v4}, Lcom/amazon/device/ads/cr;->b()Lcom/amazon/device/ads/cs;

    move-result-object v4

    if-eqz v0, :cond_3

    sget-object v0, Lcom/amazon/device/ads/cr$a;->l:Lcom/amazon/device/ads/cr$a;

    :goto_1
    invoke-virtual {v4, v0, v2, v3}, Lcom/amazon/device/ads/cs;->a(Lcom/amazon/device/ads/cr$a;J)V

    move v0, v1

    goto :goto_0

    :cond_2
    iget-object v1, p0, Lcom/amazon/device/ads/az;->g:Lcom/amazon/device/ads/cr;

    invoke-virtual {v1}, Lcom/amazon/device/ads/cr;->b()Lcom/amazon/device/ads/cs;

    move-result-object v1

    sget-object v2, Lcom/amazon/device/ads/cr$a;->n:Lcom/amazon/device/ads/cr$a;

    invoke-virtual {v1, v2}, Lcom/amazon/device/ads/cs;->a(Lcom/amazon/device/ads/cr$a;)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    goto :goto_0

    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0

    :cond_3
    :try_start_2
    sget-object v0, Lcom/amazon/device/ads/cr$a;->m:Lcom/amazon/device/ads/cr$a;
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_0

    goto :goto_1
.end method
